...
class myTurtle (threading.Thread):
    def __init__(self, x, y):
        ...
        self.time=0
        self.timeH=0
        self.chanceH=random.randint(0,100)
        self.imune=False
        self.mask=False
...
def initturtles():
    ...
    for t in range(NOPEOPLE):
        ...
        if random.randint(0,100)<=4 and x[t].mask==False:
            x[t].mask=True
            x[t].chance=0
            x[t].noel.color("yellow","white")
        PEOPLE[r].append(x[t])
        if t == rr:
...
def drawLocations(FONT):
    ...
    draw_rectangle(drawer, -800, 0, 125, 125, 2, 'blue')
    drawer.write('Spital', align="center", font=FONT)
    ...
    for day in range(NOITERATIONS):
        ...
        for x in balls:
            if not x.infected:
                pass
                #x.runHome()
                if random.randint(0,100)>=96 and x.mask==False:
                    x.mask=True
                    x.chance=0
                    x.noel.color("yellow","white")
            elif x in PEOPLE[8]:
                    x.timeH+=1
            else:
                    pass
                    ...
                    #x.runHome()
            if x.timeH==3:
                x.noel.color('red','white')
                x.noel.shapesize(1,1,1)
                x.imune=True
                x.time=0
                x.infected=False
                x.mask=True
        ...
        for number in INFECTED:
            if len(INFECTED[number])!=0:
                for x in PEOPLE[number]:
                    if x.chance<=15 and x.infected==False:
        ...
        for t in range(len(balls)):
            ...
            #print('r=', r)
            if inf>=6:
                if balls[t].infected and balls[t].chanceH<=20:
                    INFECTED[r].append(balls[t])
                    balls[t].modify(ARR[8][0]+25, ARR[8][1]-25)
                    PEOPLE[8].append(balls[t])
                    R.add(r)
                else:
                    ...
                    balls[t].chance=random.randint(0,100)
                    PEOPLE[r].append(balls[t])
                    if random.randint(0,100)<=4 and balls[t].mask==False:
                        balls[t].mask=True
                        balls[t].chance=0
                        balls[t].noel.color("yellow","white")
                    if balls[t].infected:
                        INFECTED[r].append(balls[t])
                        R.add(r)
                        x.chanceH=random.randint(0,100)
            
        #print('INFECTED: ',INFECTED)
        #print('PEOPLE: ',PEOPLE)
        #print()
        f.write("{},{},{},{}\n".format(inf, heal, rec,dead))
        #gotolocation(PEOPLE)
...
with open("data.csv", "w") as f:
    f.write("Infected, Healthy, Recovered, Dead\n")
    #drawLocations(FONT)
